...
def calculate_shares(transactions):
    """Calculates the shares from a list of transactions"""
    shares = []
    for transaction in transactions:
        buying_costs = transaction["price"] * transaction["shares"]
        transaction["buying_costs"] = buying_costs
        if not shares:
            shares.append(transaction)
        else:
            for share in range(len(shares)):
                if shares[share]["Symbol"] == transaction ["Symbol"]:
                    shares[share]["shares"] += transaction ["shares"]
                    shares[share]["buying_costs"] += transaction ["buying_costs"]
                    break
                if share == len(shares) - 1:
                    shares.append(transaction)
    return shares